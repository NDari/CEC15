 ...
 * Nov. 16th 2014
 * Reference: J. J. Liang, B. Y. Qu, P. N. Suganthan, "Problem Definitions and Evaluation Criteria for the CEC 2015 Competition
 * on Learning-based Real-Parameter Single Objective Optimization",Technical Report201411A, Computational Intelligence
 * Laboratory, Zhengzhou University, Zhengzhou, China and Technical Report,anyang Technological University, Singapore,2014.
 *
 */
...
extern double *OShift,*M,*y,*z,*x_bound,*bias;
...
void cec15_test_func(double *x, double *f, int nx, int mx,int func_num)
{
    ...
    if (ini_flag==0)
    {
        ...
        if (cf_num>1)
        {
            sprintf(FileName, "input_data/bias_%d.txt", func_num);
            fpt = fopen(FileName,"r");
            if (fpt==NULL)
            {
                printf("\n Error: Cannot open input file for reading \n");
